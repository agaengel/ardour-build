...
src expat-2.4.7 tar.gz https://sourceforge.net/projects/expat/files/expat/2.4.7/expat-2.4.7.tar.gz
autoconfbuild
...
cmake -DLWS_WITH_SSL=OFF -DLWS_WITH_EXTERNAL_POLL=ON -DMINGW=ON \
	-DLWS_WITHOUT_TEST_SERVER=ON -DLWS_WITHOUT_TESTAPPS=ON -DDISABLE_WERROR=ON -DLWS_WITH_EVLIB_PLUGINS=OFF \
	-DCMAKE_SYSTEM_NAME=Windows -DCMAKE_C_COMPILER=`which ${XPREFIX}-gcc` -DCMAKE_CXX_COMPILER=`which ${XPREFIX}-g++` -DCMAKE_RC_COMPILER=`which ${XPREFIX}-windres` \
...
if test ! -d ${SRCDIR}/soundfind.git.reference; then
	git clone --mirror https://github.com/danieldram/soundfind.git ${SRCDIR}/soundfind.git.reference
fi
cd ${BUILDD}
#git clone --reference ${SRCDIR}/soundfind.git.reference --depth 1 https://github.com/danieldram/soundfind.git || true
